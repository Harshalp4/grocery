...
class _CheckoutPageState extends ConsumerState<CheckoutPage> {
  ...
  int _payment = 0;
  bool _placing = false;
  String? _selectedAddressId; // null = use the default saved address
  ...
  Widget build(BuildContext context) {
    ...
    final toPay = Pricing.payable(itemTotal, discount);
    final slotLabels = slots.valueOrNull ?? const <String>[];

    // Serviceability of the chosen address's pincode (checked only when known).
    ...
    final service = hasPincode ? ref.watch(serviceableProvider(pincode)) : null;
    final serviceable = service?.valueOrNull?.serviceable ?? true;
    final canPlace = signedIn &&
        resolved != null &&
        cart.lines.isNotEmpty &&
        slotLabels.isNotEmpty &&
        serviceable;
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          AppCard(
            child: Column(
              children: [
                ...
                if (discount > 0)
                  BasketLineTile(
                      title: 'Coupon (${cart.couponCode})',
                      trailing: '– ₹$discount'),
                BasketLineTile(
                    title: 'Delivery charges',
                    trailing: deliveryFee == 0 ? 'FREE' : '₹$deliveryFee'),
                Container(
          ...
          ElevatedButton(
            onPressed: (_placing || !canPlace) ? null : _placeOrder,
            child: _placing
                ? const SizedBox(
                    height: 20,
                    width: 20,
      ...
      return AppCard(
        child: Column(
          ...
          children: [
            ...
            Text('Please sign in to add a delivery address and place your order.',
  ...
  Future<void> _placeOrder() async {
    ...
    if (user == null || resolved == null || slotLabels.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content:
                Text('Please sign in and add a delivery address to continue')),
      );
      return;
    }
    final address = resolved.formatted;
    final slot = slotLabels[_slot.clamp(0, slotLabels.length - 1)];
    final discount = cart.discount;
    final deliveryFee = Pricing.deliveryFee(cart.itemTotal);
    final total = Pricing.payable(cart.itemTotal, discount);
    final input = OrderInput(
      customerName: user.name?.isNotEmpty == true ? user.name! : 'Customer',
      phone: user.phone,
      address: address,
      slot: slot,
