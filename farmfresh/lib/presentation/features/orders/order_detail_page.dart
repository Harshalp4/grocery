...
/// Line total = unit price (parsed from the "₹95" label) × unit count.
int _lineTotal(BasketLine item) {
  final digits = item.priceLabel.replaceAll(RegExp(r'[^0-9]'), '');
  final unit = digits.isEmpty ? 0 : int.parse(digits);
                ...
                child: Column(
                  children: [
                    for (var i = 0; i < o.items.length; i++)
                      BasketLineTile(
                        title: o.items[i].qty > 1
                            ? '${o.items[i].name} · ${o.items[i].quantity} × ${o.items[i].qty}'
                            : '${o.items[i].name} · ${o.items[i].quantity}',
                        trailing: '₹${_lineTotal(o.items[i])}',
                        showDivider: i != o.items.length - 1,
                      ),
                    const Divider(height: 18),
                    if (o.itemTotal > 0)
                      BasketLineTile(
                          title: 'Item total', trailing: '₹${o.itemTotal}'),
                    if (o.savings > 0)
                      BasketLineTile(
                          title: 'Coupon${o.couponCode != null ? ' (${o.couponCode})' : ''}',
                          trailing: '– ₹${o.savings}'),
                    if (o.itemTotal > 0)
                      BasketLineTile(
