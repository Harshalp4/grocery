            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _ProductScroller(
                    products: products,
                    // Real deals: biggest saving vs market price first.
                    select: (l) => (l.where((p) => p.savingsPercent > 0).toList()
                      ..sort((a, b) =>
                  ...
                  _ProductScroller(
                    products: products,
                    select: (l) =>
                        l.where((p) => p.tags.contains('fresh')).toList(),
                  ),
                  SectionHeader(
                    'Best Sellers',
                    actionLabel: 'More',
                  ...
                  _ProductScroller(
                    products: products,
                    select: (l) =>
                        l.where((p) => p.tags.contains('best')).toList(),
                  ),
        ...
        Stack(
          ...
          children: [
            IconButton(
              ...
              icon: Icon(Icons.notifications_outlined, color: c.gold),
            ),
            if (unread > 0)
              Positioned(
                right: 4,
...
class _ProductScroller extends StatelessWidget {
  const _ProductScroller({required this.products, required this.select});
  final AsyncValue<List<Product>> products;
  final List<Product> Function(List<Product>) select;
...
/// Horizontally-scrolling promo banners from the admin CMS (`GET /banners`).
class _BannerStrip extends ConsumerWidget {
  const _BannerStrip();
        ...
        child: ListView.separated(
          ...
          itemBuilder: (_, i) {
            ...
            return GestureDetector(
              onTap: () {
                if (b.actionType == 'url' &&
                    (b.actionValue?.isNotEmpty ?? false)) {
                  context.go(b.actionValue!);
                } else {
                  context.go('/products');
                }
              },
              child: Container(
                ...
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    colors: [c.green, const Color(0xFF1F4A30)],
                  ),
                  borderRadius: AppRadius.smAll,
                ),
                child: Column(
